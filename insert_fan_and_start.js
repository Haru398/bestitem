...
const newest = files.slice(0, 4).reverse();

newest.forEach((f, i) => {
  const dest = path.join('D:\\서버구축폴더\\bestitem\\public\\images', `fan_${i + 1}.png`);
  fs.copyFileSync(f.fullPath, dest);
  console.log(`Copied ${f.name} to fan_${i + 1}.png`);
});
...
복잡한 기능 없이 딱 필요한 기능만 알차게 담은 가성비 끝판왕 스탠드 선풍기! 올여름 폭염이 오기 전에 하단 링크를 통해 미리 장만해 보세요!`,
  productLink: 'https://link.coupang.com/a/eD9OrsAenA',
  iframeLink: '<iframe src="https://coupa.ng/cntcTB" width="120" height="240" frameborder="0" scrolling="no" referrerpolicy="unsafe-url" browsingtopics></iframe>'
};

const stmt = db.prepare('INSERT INTO posts (id, category, subCategory, title, content, productLink, iframeLink, createdAt) VALUES (@id, @category, @subCategory, @title, @content, @productLink, @iframeLink, CURRENT_TIMESTAMP)');
stmt.run(newPost);
