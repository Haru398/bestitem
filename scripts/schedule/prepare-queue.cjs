...
function makePost(item, order, imagePath) {
  ...
  const description = `${titleBase} 구매 전 확인 항목은 ${focus.join(', ')}입니다. 판매 페이지의 제품 정보와 제조사 공식 제품·지원 자료를 기준으로 옵션, 호환 조건과 추가 비용을 정리했습니다.`;
  const intro = `‘${titleBase}’ 선택을 ${frame.label}에 맞춰 검토할 때 확인할 순서는 ${focus[0]}, ${focus[1]}, ${focus[2]}입니다. 현재 판매 페이지의 제품명은 ‘${item.productName}’로 표시되지만, 판매 제목만으로 선택한 용량, 색상, 구성품과 국내 지원 범위를 모두 확정할 수는 없습니다. 이 글은 판매 페이지를 되풀이하거나 성능 순위를 매기는 글이 아니라, 검색 단계에서 결제 단계로 넘어가기 전에 틀리기 쉬운 항목을 줄이기 위한 구매 점검표입니다. 각 항목을 판매 옵션과 제조사 공식 자료에서 대조하고, 같은 분야의 다른 제품은 [관련 카테고리 글](/category/${item.category}/)에서도 비교할 수 있습니다.`;
  const sections = [
    {
      heading: `1. ${titleBase} 제품명·모델번호 확인하기`,
      body: `‘${titleBase}’ 구매 전에는 정확한 모델번호와 선택 옵션을 먼저 확인해야 합니다. 현재 판매 페이지에는 ‘${item.productName}’로 표시되어 있습니다. 검색할 때 입력한 표현은 ‘${item.query}’입니다. 핵심 이름은 맞지만, 긴 판매 제목에는 색상·용량·번들·유통 방식이 한꺼번에 들어갈 수 있습니다. ${signalText} ${guide.option} 결제 화면에서 선택된 옵션을 바꿀 때 상품 이미지와 가격만 보지 말고 모델번호, 기본 구성품, 판매 주체를 다시 읽으세요. 제조사 공식 제품 페이지에서 같은 이름을 찾은 뒤 판매 제목에 없는 차이를 기록하면 다른 세대나 호환품을 고를 가능성을 줄일 수 있습니다. 이 과정에서 일치하지 않는 항목이 하나라도 있으면 링크가 맞더라도 주문을 보류하는 편이 안전합니다.`,
    },
    {
      heading: `2. 사용 목적에서 ${focus[0]} 확인하기`,
    ...
    {
      heading: `3. 연결 기기와 공간을 기준으로 ${focus[1]} 확인하기`,
    ...
    {
      heading: `4. 본체 가격에 ${focus[2]}·유지비 더하기`,
      body: `${guide.cost} 쿠팡에 표시되는 가격과 배송 조건은 바뀔 수 있으므로 이 글에는 특정 가격을 고정하지 않습니다. 대신 처음 한 달에 필요한 부속품, 1년 동안 교체할 소모품, 설치와 서비스 비용을 나눠 적어 보세요. ${frame.cost} 번들 상품은 각 구성품의 정확한 품번과 수량을 확인하고, 사은품은 반품 시 함께 반환해야 하는지도 살펴봐야 합니다. 판매 제목에 원하는 부속이 보이더라도 선택 옵션을 바꾸면 빠질 수 있습니다. 제조사 보증과 판매자 자체 보증의 접수처가 같은지도 결제 전에 확인하세요.`,
    },
    {
      heading: `5. 결제 직전 확인할 네 가지`,
      body: `첫째, 상품 페이지의 모델번호와 제조사 공식 페이지의 모델번호가 같은지 확인합니다. 둘째, 현재 선택된 색상·용량·번들과 이미지가 일치하는지 봅니다. 셋째, 배송·설치 가능 지역과 반품비, 공식 서비스 접수 경로를 읽습니다. 넷째, 쿠팡 링크를 다시 열어 품절이나 판매자 변경으로 옵션이 바뀌지 않았는지 점검합니다. 제휴 링크는 판매 페이지로 이동하는 출발점이지 모든 옵션을 보증하는 문서가 아닙니다. 공식 지원 페이지에서 설명서와 호환표를 찾을 수 없는 모델이라면 구매를 서두르지 마세요. 이 네 단계가 모두 맞을 때만 현재 목적에 맞는 후보라고 판단하는 편이 좋습니다.`,
    },
  ];
  const conclusion = `${titleBase}처럼 이름이 널리 알려진 제품군도 판매 옵션과 사용 환경을 맞추는 과정이 필요합니다. 판매 페이지의 제품명과 선택한 옵션이 본품을 가리키는지, ${focus.join(', ')} 항목이 원하는 조건에 맞는지 순서대로 확인하세요. 제조사 제품·지원 페이지에서 모델번호가 확인되고 구성품과 추가 비용까지 받아들일 수 있을 때 주문하는 것이 안전합니다. 반대로 모델번호, 공식 지원이나 옵션 구성이 하나라도 모호하다면 비슷한 이름에 끌려 결제하기보다 판매자에게 정확한 품번을 확인한 뒤 비교를 이어가는 편이 낫습니다.`;
  ...
  return {
    ...
    heroImage: imagePath,
    heroImageAlt: `${item.productName} 상품 이미지`,
    heroImageCaption: `판매 페이지에서 확인한 ${item.productName}`,
    heroImageCredit: '쿠팡 파트너스 제공 이미지',
    ...
    editorial: {
      status: 'reviewed',
      basis: `쿠팡 파트너스가 제공한 상품명·상품번호·이미지·개별 연결 URL과 ${source.publisher} 공식 제품 및 지원 페이지를 분리해 대조하고, 확인되지 않은 가격·성능 수치는 쓰지 않았습니다.`,
      lastChecked: checkedAt,
      caution: '판매 제목은 선택 옵션 전체를 보증하지 않습니다. 모델번호, 구성품, 국내 서비스와 가격은 결제 직전 판매 페이지와 제조사 공식 자료에서 다시 확인해야 합니다.',
    },
    verdict: {
      oneLine: `직접 확인할 항목은 ${focus.join('·')}입니다. 판매 제목만으로 정확한 옵션과 호환을 단정하면 안 되는 상품입니다.`,
    ...
    faq: [
      ...
      { question: `대표 상품 이미지에 보이는 구성품이 모두 포함되나요?`, answer: '대표 이미지는 연출이나 여러 옵션을 함께 보여줄 수 있습니다. 포함 여부는 선택한 옵션의 구성품 표와 판매자 안내를 기준으로 판단해야 합니다.' },
