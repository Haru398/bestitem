...
const guideId = 'guide-skhynix-p41-heatsink';

const stmt = db.prepare(`
  INSERT INTO guides (id, title, summary, seoTitle, metaDescription, content, relatedGuides)
  VALUES (?, ?, ?, ?, ?, ?, ?)
`);

const content = `
게이머와 작업자들 사이에서 '끝판왕'이라 불리는 PCIe 4.0 SSD가 있습니다. 바로 **SK하이닉스 PLATINUM P41 M.2 NVMe**입니다. 최대 읽기 속도 7,000MB/s를 자랑하며, 안정성 면에서도 검증된 최고의 제품입니다.
...
**SK하이닉스 PLATINUM P41**은 현존하는 최고의 PCIe 4.0 SSD 중 하나입니다. 하지만 그 압도적인 성능을 유지하려면 적절한 온도 관리가 필수적입니다. 메인보드에 기본 방열판이 있다면 즉시 구매하셔도 좋고, 없다면 저렴한 사제 방열판을 묶어서 구매하시는 것을 강력히 권장합니다.

<iframe src="https://coupa.ng/cnKAZ1" width="120" height="240" frameborder="0" scrolling="no" referrerpolicy="unsafe-url" browsingtopics></iframe>
`;

try {
  stmt.run(
    guideId,
    'SK하이닉스 P41 방열판 꼭 필요한가? 발열 원인 및 메인보드 호환성 가이드',
    '최고 성능의 PCIe 4.0 SSD인 SK하이닉스 P41. 방열판 없이 써도 될까요? 발열 원인과 메인보드 호환성, 노트북 장착 시 주의사항까지 완벽하게 정리했습니다.',
    'SK하이닉스 P41 방열판 꼭 필요한가? 발열 원인 및 메인보드 호환성 완벽 가이드',
    '최고 성능의 PCIe 4.0 SSD인 SK하이닉스 P41. 방열판 없이 써도 될까요? 발열 원인, 메인보드 호환성, 노트북 장착 시 주의사항까지 완벽하게 정리했습니다.',
    content,
    '[]'
  );
