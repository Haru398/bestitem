...
많은 분들이 의류 구매 시 가장 고민하시는 부분이 바로 '사이즈'와 '실제 착용 느낌'일 것입니다. 저희가 직접 촬영한 아래 사진의 피팅 모델은 **키 180cm, 몸무게 72kg의 체형으로 'L 사이즈'를 여유롭게 착용**하였습니다. 일반적인 체형의 남성분이라면 정사이즈를 추천해 드리며, 조금 더 힙하고 루즈한 오버핏을 원하신다면 한 사이즈 업(Up) 하시는 것을 적극 권장합니다. 블랙, 그레이, 네이비 등 베이직한 컬러 구성으로 어떤 상의와 매치해도 찰떡같이 어울리는 놀라운 소화력을 보여줍니다.
...
*(본 포스팅에 사용된 모델 착용 컷 및 제품 일부 이미지는 제품의 핏과 외관에 대한 소비자의 시각적 이해를 돕기 위해 AI 기술을 활용하여 실제 상품과 가장 유사하게 재현 및 생성된 참고용 이미지입니다. 저작권에 위배되지 않는 안전한 이미지만을 사용하였습니다.)*`;

const newItem = {
  id: 'item-shorts-1',
  title: '[1+1] 버닝몬스터 데일리 무지 밴딩 반바지 하프팬츠 (사이즈표 포함)',
  category: '패션/의류',
  content: content,
...
console.log('Successfully added item-shorts-1 to data.json with new model info and sizes.');
